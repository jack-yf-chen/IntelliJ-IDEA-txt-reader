...
- 恢复阅读位置时优先使用 `globalOffset + anchorText`，不受窗口大小、字体、加粗、行距和阅读宽度变化影响。
- 窗口大小、字号、行距、字体、加粗或阅读宽度变化前，先锁定当前可视阅读锚点；连续缩放时延迟整本重排，重排期间暂停滚动条回写，重排完成后按全局阅读锚点重新定位，避免像素滚动值导致位置漂移。
