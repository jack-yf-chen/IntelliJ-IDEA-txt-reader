...
- 正文采用整本书连续渲染，章节之间不再替换正文内容，读者可以在同一屏同时看到上一章尾部和下一章开头。
...
- 打开 TXT 后将整本书内容一次性渲染到阅读区，保持滚动条在全书范围内连续移动。
- 章节下拉框、上一章和下一章按钮只负责定位到对应章节起点，不再重新替换正文内容。
- 滚动过程中根据可视区域顶部对应的文本偏移自动更新当前章节、章节进度和全书进度。
