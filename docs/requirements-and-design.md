...
- 打开 TXT 后不再将整本书一次性渲染到阅读区，而是生成章节渲染窗口。
- 渲染窗口由当前章节全文、上一段上下文和下一段上下文组成，默认上下文长度为 5000 字。
- 章节下拉框、上一章和下一章按钮负责切换渲染窗口并定位到对应章节。
- 滚动过程中根据可视区域内 25% 高度处的阅读锚点自动更新当前章节、章节进度和全书进度。
- 当阅读锚点进入相邻章节上下文时，只更新章节下拉框、进度和阅读锚点，不立即重建渲染窗口，避免上下文预览区触发来回跳转。
- 鼠标滚轮或键盘滚动到达渲染窗口真实顶部或底部时，会按视口边缘文本所在章节切换窗口，并在切换完成后延续一次同方向滚动，避免章节窗口边界导致滚轮空转，同时避免旧窗口章节状态把阅读位置拉回上一章。
- 跳转到上一章末尾时按章节结束偏移对齐，方便回看上一章尾部。
- 恢复阅读位置时优先使用 `globalOffset + anchorText`，不受窗口大小、字体、行距和阅读宽度变化影响。
- 窗口大小、字号、行距或阅读宽度变化后，延迟按全局阅读锚点重新定位，避免像素滚动值导致位置漂移。
