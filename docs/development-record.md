...
- 2026-09-18：将插件版本号提升到 `0.4.4`。修复 Neat Reader 网页在窄工具窗口内显示不完整、缩放按钮数值变化但网页版式未实际收缩的问题：不再依赖 JCEF 浏览器级缩放，改为在页面内注入 viewport、横向溢出控制和宽度测量脚本，根据工具窗口宽度对网页正文做页面级自适应。
...
- 确认 Neat Reader Tab 会随工具窗口宽度自动适配网页版式，缩小、自适应和放大按钮可用，页面不应出现主要内容被横向裁剪。
