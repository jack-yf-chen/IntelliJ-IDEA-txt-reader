...
- 右侧 `Novel Reader` 工具窗口，包含“本地阅读”“书架”和“Neat Reader”三个 Tab。
- **书架**：「最近阅读」+「我的收藏」两个分区，使用自动换行的书籍网格卡片；卡片常驻显示放大的封面、最多两行书名和最后阅读时间（`3 小时前` / `昨天` / `2026-09-21`），收藏 / 移除放在卡片底部右侧，阅读进度、格式和完整路径通过鼠标悬浮查看；
  点卡片续读、点 ★ 收藏 / 取消收藏、点 ✕ 从历史移除，另有「打开书籍」与「清空历史」（只清未收藏的）。
  书架是**应用级**的（存在 `bookshelf.xml`），换工程不丢；封面在后台单线程懒加载，最长边 320 px 的缩略图缓存
  在 IDEA 系统缓存目录，文件名用 `sha1(path)_size_mtime.png` 做指纹，书被替换后自动失效。
