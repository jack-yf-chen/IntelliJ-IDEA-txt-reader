...
- EPUB 文件读取：解析 `container.xml` / OPF manifest / spine，按阅读顺序提取 XHTML 正文。
- EPUB 编码加固：编码探测按「BOM → UTF-8 严格解码 → 声明编码白名单 → GB18030 → Big5 → UTF-8 兜底」的强信号优先顺序执行；
  对**书本身被二次转码过**的乱码（`ä½œä¸º…`）按反向映射还原；DOM 解析改走字符流，避免被文件里陈旧的 `encoding=` 声明二次解码。
...
- **EPUB 由目录驱动**：优先读 `nav.xhtml`、没有则读 `toc.ncx`。两种都支持是硬要求 —— 只认 nav 会在只有 NCX 的书上直接退化。
  层级按「每层条目数落在 `[8,120]` 取最深合格层，再并上更浅层里的叶子条目」自动选择，不用人工指定。
- **章节标题取自目录**，不再用 spine 文件名或 `h1`/`title` 猜；裸的「§1」会按父标题补全成「第一篇 世界作为表象初论 · §1」。
  目录缺失时自动降级为「一篇 spine 一章」。
- **TXT 用规则表识别**：8 条规则分强弱。强规则（`第N章/回/节/卷/集/部/篇`、`卷N/篇N/章N`、英文 `Chapter/Part/Prologue/…`、
  独立标题词、Markdown `#`）命中即认；弱规则（中文序号 `一、`、阿拉伯序号 `1.`、罗马数字 `IV.`）
  额外要布局证据（本行是首行 / 上一行是空行 / 上一行以句末标点结尾），从而不把正文里的编号列表当成章。
- **章节区间连续覆盖 `0..全文长度`**，不留无处归属的空洞：短前导把首章起点钳到 0，长前导才补一个「开头」章。
- 上一章 / 下一章 / 章节下拉框导航。

### 脚注

- 两条识别通道：标注通道（`epub:type` / `class` / `id` 带脚注语义）与互指锚点通道（双向 `#frag` 配对）。
  互指通道用**结构性判据**（注释条目那个锚点必须领衔其所在块）判定，**不要求两端编号相等**
  —— 很多转换器给注释条目的编号是按小节重新起算的，要求相等会整片漏掉。
- **注释区容器会继续下钻**：`<section id="footnotes">` 这类外层容器不再抢命中，里面的 `<aside id="fnN">` 每条单独成一条注释。
- 正文脚注引用显示为可点击的 `[注N]`，弹窗直接显示注释正文、不做跳转；
  弹窗标题保留书里原本的标记（如 `[1]`），便于和插件编号对不上时排查。
- **注释按章归属**：本章首次引用到的注释写在本章末尾【注释】，从没被引用过的在文档末尾兜底；
  配对键是 `文档#id` 全书唯一，跨文档复用 id 的老书不会串章显示错注释。
...
- `0.9.0` 是 breaking change：EPUB 章节改由目录驱动后 `plainText` 有变化（增量全部是章节边界的空行和 `[注N]` 标记，正文内容不变），
  旧版本保存的阅读位置会漂移几行，靠原文锚点可恢复到正确文字附近。
- EPUB 目录层级是自动选择的（`[8,120]` 取最深合格层）。个别书若层级特殊可能偏粗或偏细，
  例如某本只有 4 条二级条目的书会按规则合并到父章节（内容不丢，只是不单独成章）。
